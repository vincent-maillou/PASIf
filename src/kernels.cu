 ...
 */
 ...
 void SpTdV(reel *d_val_G, 
  ...
  if(blockIdx.x==0 && threadIdx.x==0){
    uint k=0;
    while(k<nzz_G){      
      atomicAdd(&Y[d_slice_G[k]],  d_val_G[k]*X_source[d_row_G[k]]*X_source[d_col_G[k]]);
      k++;
    }
    ...
    k=0;
    while(k<nzz_L){
      atomicAdd(&Y[d_hyperslice_L[k]], d_val_L[k]*X_source[d_slice_L[k]]*X_source[d_row_L[k]]*X_source[d_col_L[k]]);
      k++;
    }
  }
 }
